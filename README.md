...
- Host mutations are tracked as full-layout, localized-layout, or paint-only work. Paint-only changes reuse cached cell geometry; geometry and text changes inside retained absolute-positioned islands recompute only those isolated subtrees. Both incremental paths reraster the union of affected old/new subtree rows into compact, terminal-independent Canvas runs. Flow layout, tree, style-table, and interaction changes retain the full-layout oracle. The backend—not the game—owns `CanvasHandle.present()`, PTX1 transport, and the handoff to Rust damage tracking.
...
- Resize readiness currently uses a native 250 ms `TIOCGWINSZ` change probe rather than an edge-triggered SIGWINCH producer. Unchanged probes stay entirely native and do not wake JavaScript. PocketJS can locally reflow independent absolute subtrees and reraster affected rows, but general local Flex reflow and sparse Canvas patch opcodes remain future work; Canvas still submits bounded full semantic frames before native row damage limits terminal output.
