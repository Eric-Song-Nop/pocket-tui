...
- Host mutations are tracked as full-layout or paint-only work. Paint-only changes reuse cached cell geometry and reraster affected subtree rows into compact, terminal-independent Canvas runs; geometry and text changes retain a full-layout oracle. The backend—not the game—owns `CanvasHandle.present()`, PTX1 transport, and the handoff to Rust damage tracking.
...
- Resize readiness currently uses a native 250 ms `TIOCGWINSZ` change probe rather than an edge-triggered SIGWINCH producer. Unchanged probes stay entirely native and do not wake JavaScript. Paint-only PocketJS frames reuse layout and reraster affected rows, but local Flex reflow and sparse Canvas patch opcodes remain future work; Canvas still submits bounded full semantic frames before native row damage limits terminal output.
